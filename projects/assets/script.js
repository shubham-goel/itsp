...
function exampleClickToSave(area) {
    ...
    var data={};
    data[area]=$("#"+area).html();
    $.post("php/savefile.php?by="+id,data)
    .done(function(data){
        console.log(data);
                    ...
                    if(name=='jpg'||name=='jpeg'||name=='png'||name=='gif'||name=='icon'||name=='jfif'||name=='JPG')
                    {
                        ...
                        else
                        {
                             ...
                             var txt= "?by="+$("#uploadedby"+i).val()+"&no="+i;
                                $.ajax({
                                    url: 'php/img-upload.php'+txt,  //Server script to process data
                                    type: 'POST',
                                    //Ajax events
            ...
            e.preventDefault();
                     var formData = new FormData($('#projectimg')[0]);
                             var txt= $("#projectimg").serialize();
                                $.ajax({
                                    url: 'php/projectimg-upload.php?'+txt,  //Server script to process data
                                    type: 'POST',
                                    //Ajax events
                                    success: function(data){
                                        if(data=="done")
